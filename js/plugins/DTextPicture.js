...
(function () {
  ...
  Game_Screen.prototype.setDTextWindowCursor = function (
    pictureId,
    rect,
    switchId
  ) {
  ...
  Sprite_Picture.prototype.makeDynamicBitmap = function () {
    ...
    this.bitmap = tempWindow.createTextContents(
      this.picture().getDText(),
      this.dTextInfo
    );
    ...
    if (this.dTextInfo.windowFrame) {
      ...
      this.makeFrameWindow(
        this.bitmap.width * scaleX,
        this.bitmap.height * scaleY
      );
    }
  };
      ...
      switch (code) {
        case "OC":
          ...
          this.changeOutlineColor(
            !isNaN(colorIndex) ? ColorManager.textColor(colorIndex) : colorCode
          );
          break;
        case "OW":
