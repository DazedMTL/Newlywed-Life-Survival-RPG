...
Moghunter.ghud_number_pos_x = Number(
  Moghunter.parameters["Number X-Axis"] || 240
);
Moghunter.ghud_number_pos_y = Number(
  Moghunter.parameters["Number Y-Axis"] || 24
);
Moghunter.ghud_fade_limit = Number(Moghunter.parameters["Fade Max"] || 60);
Moghunter.ghud_hudvisible = String(
  Moghunter.parameters["Initial Visible"] || "true"
);
...
Game_System.prototype.initialize = function () {
  ...
  this._ghud_visible =
    String(Moghunter.ghud_hudvisible) === "true" ? true : false;
};
...
//==============================
var _mog_goldHud_scnMap_snapForBattleBackground =
...
Gold_Hud.prototype.refresh_data = function () {
  ...
  this._hud_size[2] =
    Moghunter.ghud_pos_x + this._layout_img.width - $gameMap.tileWidth();
...
Gold_Hud.prototype.create_number = function () {
  this._number = [];
  this._number_img_data = [
    this._number_img.width,
...
// * Refresh Number
//==============================
Gold_Hud.prototype.refresh_number = function (
  sprites,
  value,
  img_data,
  x,
  ...
  if (this.is_hud_visible()) {
    ...
    if ($gameMessage.isBusy()) {
      ...
      if (this.opacity > Moghunter.ghud_fade_limit) {
        this.opacity -= 10;
        if (this.opacity < Moghunter.ghud_fade_limit) {
          this.opacity = Moghunter.ghud_fade_limit;
        }
      }
    }
  }
...
Gold_Hud.prototype.update = function () {
  ...
  if (this._hud_size[0] === -1 && this._layout_img.isReady()) {
    this.refresh_data();
  }
