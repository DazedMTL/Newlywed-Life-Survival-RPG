...
Game_Interpreter.prototype.trPopupType = function (type, params) {
  if (type === 0) {
    return $dataItems[params[0]];
  }
  if (type === 1) {
    return $dataWeapons[params[0]];
  }
  if (type === 2) {
    return $dataArmors[params[0]];
  }
  return null;
};
...
//==============================
var _mog_trspop_sprtBase_createLowerLayer =
...
TreasureIcons.prototype.setupNew = function (data) {
  ...
  this._cx =
    data[2] -
    (ImageManager.iconWidth + 12 + wd) / 2 +
    Moghunter.trpopup_X +
    this.screenX();
  this._cy =
    data[3] - ImageManager.iconHeight + Moghunter.trpopup_Y + this.screenY();
