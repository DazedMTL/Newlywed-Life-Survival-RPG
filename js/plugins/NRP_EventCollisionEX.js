...
(function () {
  ...
  Game_CharacterBase.prototype.posCollisionEX = function (x, y) {
    return (
      this._x + this._collisionRight >= x &&
      this.x - this._collisionLeft <= x &&
      this._y + this._collisionDown >= y &&
      this._y - this._collisionUp <= y
    );
  };
  ...
  Game_Event.prototype.eventsXyNt = function (x, y) {
    ...
    return $gameMap
      .events()
      .filter(
        (event) => event != this && event.posNtCollisionEXEvent(x, y, this)
      );
  };
  ...
  if (pConsiderMove) {
    ...
    Game_Event.prototype.isCollidedWithPlayerCharacters = function (x, y) {
      ...
      return (
        $gamePlayer.posCollisionEXEvent(x, y, this) ||
    ...
    Game_Followers.prototype.isSomeoneCollidedExEvent = function (x, y, event) {
      return this.visibleFollowers().some((follower) =>
        follower.posCollisionEXEvent(x, y, event)
      );
    };
  }
  ...
  if (pConsiderMove) {
    ...
    Game_Event.prototype.isMapValid = function (x, y) {
      ...
      if ($gameMap.isLoopHorizontal()) {
        ...
        x - this._collisionLeft >= 0 &&
        x + this._collisionRight < $gameMap.width()
      ) {
        horizontalOk = true;
      }
      ...
      if ($gameMap.isLoopVertical()) {
        ...
        y - this._collisionUp >= 0 &&
        y + this._collisionDown < $gameMap.height()
      ) {
        verticalOk = true;
      }
    ...
    Game_CharacterBase.prototype.isMapPassable = function (x, y, d) {
      ...
      return (
        this.isPassableWide(tipX, tipY, d) &&
        this.isPassableWide(tipX2, tipY2, d2)
      );
    };
  ...
  if (pConsiderMove) {
    ...
    if (
      ...
      Game_Event.prototype.checkEventTriggerTouchFront = function (d) {
        Game_CharacterBase.prototype.checkEventTriggerTouchFront.apply(
          this,
          arguments
        );
      };
     ...
     * ※Game_CharacterBaseの関数をオーバーライド
     */
    const _Game_Event_checkEventTriggerTouchFront =
      Game_Event.prototype.checkEventTriggerTouchFront;
    Game_Event.prototype.checkEventTriggerTouchFront = function (d) {
      mTmpD = d;
     ...
     * 【独自】イベントから接触による起動判定２
     */
    Game_Event.prototype.checkEventTriggerTouchCollisionEX = function (
      x,
      y,
      d
    ) {
      if (!$gameMap.isEventRunning()) {
        if (
          this._trigger === 2 &&
          $gamePlayer.posTriggerTouchCollisionEX(x, y, d, this)
        ) {
     ...
     * ※当たり判定の拡張を考慮
     */
    Game_Player.prototype.posTriggerTouchCollisionEX = function (
      x,
      y,
      d,
      event
