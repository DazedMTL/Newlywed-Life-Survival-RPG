...
(function () {
  ...
  function getFieldInfo(mapId) {
    ...
    const fieldInfo = pFieldMapList.find((fieldMap) =>
      isValidField(fieldMap, mapId)
    );
    if (fieldInfo) {
      // 値を加工
  ...
  Scene_SelectSpots.prototype.loadDataMap = function () {
    ...
    for (const fieldMap of pFieldMapList) {
      ...
      if (
        fieldMapIdArray.includes(mapId) &&
        !mMapsInfo.some((m) => m.mapId == mapId)
      ) {
        // マップデータをロード
  ...
  Windows_MapDisplay.prototype.drawMap = function (item) {
    ...
    // ※縦と横の小さいほうに合わせる。
    // ※1よりは小さくならない。
    this._drawScale = Math.min(
      this.innerWidth / bitmap.width,
      this.innerHeight / bitmap.height,
      1
    );
  ...
  Windows_MapDisplay.prototype.drawSymbol = function (x, y, symbol) {
    ...
    const dx =
      this._drawMapX +
      (this._drawMapWidth * x) / this._mapInfo.width +
      tileCenterX;
    const dy =
      this._drawMapY +
      (this._drawMapHeight * y) / this._mapInfo.height +
      tileCenterY;
    ...
    if (periodicTime) {
      ...
      this.contents.paintOpacity =
        (0.5 + 0.5 * Math.sin((this._timeCount / periodicTime) * Math.PI * 2)) *
        maxOpacity;
  ...
  Windows_MapDisplay.prototype.drawCurrentPoint = function () {
    ...
    if (
      !fieldMap ||
      fieldMapId != $gameVariables.value(fieldMap.currentMapIdVariable)
    ) {
      return;
    }
  ...
  if (pShowMenuCommand) {
    ...
    Scene_Menu.prototype.createCommandWindow = function () {
      ...
      this._commandWindow.setHandler(
        pTravelSymbol,
        this.commandSelectSpots.bind(this)
      );
    };
