...
(function () {
  ...
  // do nothing
  Game_BattlerBase.prototype.onChangeEquipAndState = function (
    item,
    addedSign,
    force,
    slotId
  ...
  class Game_EquipAndStateVariable {
    ...
    execute(item, addedSign, slotId) {
      param.list
        .filter((record) => this.isValidParam(record, item, slotId))
        .forEach((record) =>
          record.list.forEach((variable) =>
            this.controlVariable(variable, addedSign)
          )
        );
    }
    ...
    controlVariable(variable, addedSign) {
      ...
      if (variableId) {
        ...
        $gameVariables.setValue(
          variableId,
          prev + (addedSign ? operand : -operand)
        );
      }
